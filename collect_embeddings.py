...
CONFIG = {
    ...
    "output_root": "/scratch/arihantr/CSAI/algonauts_outputs/s1_vjepa_decode_uniform20pct_oomfixed",
...
def build_vjepa_bundle(model_name, device, config):
    ...
    processor = AutoVideoProcessor.from_pretrained(model_name, cache_dir=config["cache_dir"])
    model = AutoModel.from_pretrained(model_name, dtype=model_dtype, cache_dir=config["cache_dir"], attn_implementation="sdpa").to(device).eval()
...
def forward_batch_with_frame_retry(batch_video_t, bundle, device, valid_items, retry_schedule):
    ...
    for max_frames in retry_schedule:
        try:
            # Subsample all clips in the batch to the same frame count
            ...
            batch_out = bundle["hook_store"][0] # Shape: (Batch, Tokens, Dim)
            
            # Crop each clip in the batch to its specific time window
            feats = []
            for i, (tr_idx, c_start, c_end, i_start, i_end) in enumerate(valid_items):
                feat = crop_vjepa_by_time(batch_out[i], i_start, i_end, clip_seconds=(c_end - c_start))
                feats.append(feat)

            del x_hf, batch_out, batched_clips
            bundle["hook_store"].clear()
            empty_cuda_cache()
            ...
            bundle["hook_store"].clear()
            empty_cuda_cache()
            continue # Try the next smaller frame count for the batch
...
def extract_video_features(video_path, relative_path, bundle, device, config):
    ...
    # ... (keep the manifest and success file setup) ...
    ...
    # Helper to chunk the list into batch sizes
    def chunker(seq, size):
        return (seq[pos:pos + size] for pos in range(0, len(seq), size))

    # Process in Batches!
    for batch_selected in tqdm(
        list(chunker(selected, config["batch_size"])),
        desc=f"Batches: {Path(video_path).name}",
        leave=False,
    ):
        batch_video_t = []
        valid_items = []

        # 1. Read videos for the batch using CPU
        ...
        if not batch_video_t:
            continue

        # 2. Push the whole batch through the GPU
        batch_feats, used_frames = forward_batch_with_frame_retry(
            batch_video_t=batch_video_t,
            bundle=bundle,
            device=device,
    ...
    # ... (keep the torch.save and manifest writing logic) ...
